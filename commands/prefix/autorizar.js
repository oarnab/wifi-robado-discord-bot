    ...
    async execute(message, args) {
        ...
        if(users_permitted.includes(message.author.id)) {
            ...
            if (!permisos.permisos[comando].includes(targetId)) {
                permisos.permisos[comando].push(targetId);
                guardarPermisos(permisos);
                const replyMsg = await message.reply(`Usuario ${targetId} autorizado.`);
                setTimeout(async () => {
                try {
                    await message.delete();
                ...
                const errorMsg = await message.reply("Ese usuario ya tiene permisos.");
                setTimeout(async () => {
                try {
                    await errorMsg.delete();
                    await message.delete();
